...
class AccountBloc extends Bloc<AccountEvent, AccountState> {
  ...
  void getCurrentUser(FetchCurrentUserEvent event, Emitter<AccountState> emit) async {
    emit(AccountLoading());
    try{
      ...
      if (e is DioException) {
        if (e.response != null) {
          ...
          if (statusCode == 522) {
          emit(AccountLoadFailed(message: 'Connection timed out. Please try again later'));
    ...
    try {
      ...
      if (e is DioException) {
        if (e.response != null) {
          final statusCode = e.response!.statusCode;
          if (statusCode == 522) {
            emit(LogOutFailed(message: 'Connection timed out. Please try again later'));
          } else if (statusCode == 401) {
            emit(TokenExpired());
          } else if (statusCode! >= 500 || statusCode >= 402) {
            if (e.response?.data["message"].runtimeType != String) {
