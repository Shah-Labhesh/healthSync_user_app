...
class DocProfileRepo {
  Future<Response> getDocProfile(String id) async {
    ...
    Response response = await Dio().get(AppUrls.getDoctor(doctorId: id),
  ...
  Future<Response> toggleFavourite(String id) async {
    final token = await SharedUtils.getToken();
    Response response = await Dio().post(AppUrls.toggleFavourite(doctorId: id),
        options: Options(headers: {'Authorization': 'Bearer $token'}));
    return response;
  }
  ...
  Future<Response> getDocRatings(String id) async {
    final token = await SharedUtils.getToken();
    Response response = await Dio().get(AppUrls.doctorRatings(doctorId: id),
        options: Options(headers: {'Authorization': 'Bearer $token'}));
    return response;
  }

  // create chat room
  Future<Response> createChatRoom(String id) async {
    final token = await SharedUtils.getToken();
    Response response = await Dio().post(
      AppUrls.createChatRoom,
